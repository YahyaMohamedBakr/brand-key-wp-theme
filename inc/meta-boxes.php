...
function brandkey_sector_meta_callback( $post ) {
	...
	$stats     = get_post_meta( $post->ID, '_bk_sector_stats', true );
	...
	<table class="form-table">
		...
		<tr>
			<th><label for="bk_sector_stats"><?php esc_html_e( 'الإحصائيات (JSON)', 'brandkey' ); ?></label></th>
			<td>
				<textarea id="bk_sector_stats" name="bk_sector_stats" rows="4" class="large-text code"><?php echo esc_textarea( $stats ); ?></textarea>
				<p class="description"><?php esc_html_e( 'مثال: [{"num":"98%","label":"معدل رضا المرضى"},{"num":"12K+","label":"مريض جديد شهرياً"}]', 'brandkey' ); ?></p>
			</td>
		</tr>
		<tr>
...
function brandkey_save_meta_boxes( $post_id ) {
	...
	$meta_boxes = array(
		...
		'sector'      => array( 'subtitle', 'stats', 'solutions' ),
