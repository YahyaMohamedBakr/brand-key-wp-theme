<?php
/**
 * سيكشن: services (الصفحة الرئيسية)
 *
 * @package BrandKey
 */

// لو السيكشن مش مفعّل، اخرج
if ( ! bk_section_enabled( 'front_page', 'services' ) ) return;

// محتوى السيكشن من Customizer
$title = bk_section( 'front_page', 'services', 'title' );
$desc = bk_section( 'front_page', 'services', 'desc' );
$heading = bk_section( 'front_page', 'services', 'heading' ) ?: 'h2';
$bg = bk_section( 'front_page', 'services', 'bg' );
$dark = ! empty( $bg );
$line_icon = $dark ? 'line-white.png' : 'heading-line.png';
$btn_text = bk_section( 'front_page', 'services', 'btn_text' );
$btn_url = bk_section( 'front_page', 'services', 'btn_url' );
...
<section class="services" id="services"<?php echo $style; ?>>
  <div class="services-container">

    <?php if ( 'services' !== 'hero' ) : ?>
    <header class="services-head" id="servicesHead">
      <<?php echo esc_html( $heading ); ?> class="services-title" style="color: <?php echo esc_attr( $title_color ); ?>;"><?php echo esc_html( $title ); ?></<?php echo esc_html( $heading ); ?>>
      <img src="<?php echo esc_url( BK_URI . '/assets/icons/' . $line_icon ); ?>" alt="" class="heading-line services-heading-line" aria-hidden="true" />
      <p class="services-desc" style="color: <?php echo esc_attr( $desc_color ); ?>;"><?php echo esc_html( $desc ); ?></p>
    </header>
    <?php else : ?>
