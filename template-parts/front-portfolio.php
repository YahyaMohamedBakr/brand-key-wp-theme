<?php
/**
 * سيكشن: portfolio (الصفحة الرئيسية)
 *
 * @package BrandKey
 */

// لو السيكشن مش مفعّل، اخرج
if ( ! bk_section_enabled( 'front_page', 'portfolio' ) ) return;

// محتوى السيكشن من Customizer
$title = bk_section( 'front_page', 'portfolio', 'title' );
$desc = bk_section( 'front_page', 'portfolio', 'desc' );
$heading = bk_section( 'front_page', 'portfolio', 'heading' ) ?: 'h2';
$bg = bk_section( 'front_page', 'portfolio', 'bg' );
$dark = ! empty( $bg );
$line_icon = $dark ? 'line-white.png' : 'heading-line.png';
$btn_text = bk_section( 'front_page', 'portfolio', 'btn_text' );
$btn_url = bk_section( 'front_page', 'portfolio', 'btn_url' );
...
<section class="portfolio" id="portfolio"<?php echo $style; ?>>
  <div class="portfolio-container">

    <?php if ( 'portfolio' !== 'hero' ) : ?>
    <header class="portfolio-head" id="portfolioHead">
      <<?php echo esc_html( $heading ); ?> class="portfolio-title" style="color: <?php echo esc_attr( $title_color ); ?>;"><?php echo esc_html( $title ); ?></<?php echo esc_html( $heading ); ?>>
      <img src="<?php echo esc_url( BK_URI . '/assets/icons/' . $line_icon ); ?>" alt="" class="heading-line portfolio-heading-line" aria-hidden="true" />
      <p class="portfolio-desc" style="color: <?php echo esc_attr( $desc_color ); ?>;"><?php echo esc_html( $desc ); ?></p>
    </header>
    <?php else : ?>
