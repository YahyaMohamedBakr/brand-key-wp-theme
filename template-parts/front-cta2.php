<?php
/**
 * سيكشن: cta2 (الصفحة الرئيسية)
 *
 * @package BrandKey
 */

// لو السيكشن مش مفعّل، اخرج
if ( ! bk_section_enabled( 'front_page', 'cta2' ) ) return;

// محتوى السيكشن من Customizer
$title = bk_section( 'front_page', 'cta2', 'title' );
$desc = bk_section( 'front_page', 'cta2', 'desc' );
$heading = bk_section( 'front_page', 'cta2', 'heading' ) ?: 'h2';
$bg = bk_section( 'front_page', 'cta2', 'bg' );
$dark = ! empty( $bg );
$line_icon = $dark ? 'line-white.png' : 'heading-line.png';
$btn_text = bk_section( 'front_page', 'cta2', 'btn_text' );
$btn_url = bk_section( 'front_page', 'cta2', 'btn_url' );
...
<section class="cta2" id="cta2"<?php echo $style; ?>>
  <div class="cta2-container">

    <?php if ( 'cta2' !== 'hero' ) : ?>
    <header class="cta2-head" id="cta2Head">
      <<?php echo esc_html( $heading ); ?> class="cta2-title" style="color: <?php echo esc_attr( $title_color ); ?>;"><?php echo esc_html( $title ); ?></<?php echo esc_html( $heading ); ?>>
      <img src="<?php echo esc_url( BK_URI . '/assets/icons/' . $line_icon ); ?>" alt="" class="heading-line cta2-heading-line" aria-hidden="true" />
      <p class="cta2-desc" style="color: <?php echo esc_attr( $desc_color ); ?>;"><?php echo esc_html( $desc ); ?></p>
    </header>
    <?php else : ?>
