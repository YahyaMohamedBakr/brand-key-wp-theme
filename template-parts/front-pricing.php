<?php
/**
 * سيكشن: pricing (الصفحة الرئيسية)
 *
 * @package BrandKey
 */

// لو السيكشن مش مفعّل، اخرج
if ( ! bk_section_enabled( 'front_page', 'pricing' ) ) return;

// محتوى السيكشن من Customizer
$title = bk_section( 'front_page', 'pricing', 'title' );
$desc = bk_section( 'front_page', 'pricing', 'desc' );
$heading = bk_section( 'front_page', 'pricing', 'heading' ) ?: 'h2';
$bg = bk_section( 'front_page', 'pricing', 'bg' );
$dark = ! empty( $bg );
$line_icon = $dark ? 'line-white.png' : 'heading-line.png';
$btn_text = bk_section( 'front_page', 'pricing', 'btn_text' );
$btn_url = bk_section( 'front_page', 'pricing', 'btn_url' );
...
<section class="pricing" id="pricing"<?php echo $style; ?>>
  <div class="pricing-container">

    <?php if ( 'pricing' !== 'hero' ) : ?>
    <header class="pricing-head" id="pricingHead">
      <<?php echo esc_html( $heading ); ?> class="pricing-title" style="color: <?php echo esc_attr( $title_color ); ?>;"><?php echo esc_html( $title ); ?></<?php echo esc_html( $heading ); ?>>
      <img src="<?php echo esc_url( BK_URI . '/assets/icons/' . $line_icon ); ?>" alt="" class="heading-line pricing-heading-line" aria-hidden="true" />
      <p class="pricing-desc" style="color: <?php echo esc_attr( $desc_color ); ?>;"><?php echo esc_html( $desc ); ?></p>
    </header>
    <?php else : ?>
