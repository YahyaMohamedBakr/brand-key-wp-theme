<?php
/**
 * سيكشن: packages (pricing)
 * @package BrandKey
 */
if ( ! bk_section_enabled( 'pricing', 'packages' ) ) return;

$title = bk_section( 'pricing', 'packages', 'title' );
$desc = bk_section( 'pricing', 'packages', 'desc' );
$heading = bk_section( 'pricing', 'packages', 'heading' ) ?: 'h2';
$bg = bk_section( 'pricing', 'packages', 'bg' );
$dark = ! empty( $bg );
...
$btn_text = bk_section( 'pricing', 'packages', 'btn_text' );
$btn_url = bk_section( 'pricing', 'packages', 'btn_url' );
?>

<section class="pricing-packages" id="pricing-packages"<?php echo $style; ?>>
  <div class="container" style="max-width: 1280px; margin: 0 auto; padding: 90px 24px 80px;">

    <?php if ( 'packages' === 'hero' ) : ?>
