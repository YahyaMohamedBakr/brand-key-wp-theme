<?php
/**
 * سيكشن: testimonials (الصفحة الرئيسية)
 *
 * @package BrandKey
 */

// لو السيكشن مش مفعّل، اخرج
if ( ! bk_section_enabled( 'front_page', 'testimonials' ) ) return;

// محتوى السيكشن من Customizer
$title = bk_section( 'front_page', 'testimonials', 'title' );
$desc = bk_section( 'front_page', 'testimonials', 'desc' );
$heading = bk_section( 'front_page', 'testimonials', 'heading' ) ?: 'h2';
$bg = bk_section( 'front_page', 'testimonials', 'bg' );
$dark = ! empty( $bg );
$line_icon = $dark ? 'line-white.png' : 'heading-line.png';
$btn_text = bk_section( 'front_page', 'testimonials', 'btn_text' );
$btn_url = bk_section( 'front_page', 'testimonials', 'btn_url' );
...
<section class="testimonials" id="testimonials"<?php echo $style; ?>>
  <div class="testimonials-container">

    <?php if ( 'testimonials' !== 'hero' ) : ?>
    <header class="testimonials-head" id="testimonialsHead">
      <<?php echo esc_html( $heading ); ?> class="testimonials-title" style="color: <?php echo esc_attr( $title_color ); ?>;"><?php echo esc_html( $title ); ?></<?php echo esc_html( $heading ); ?>>
      <img src="<?php echo esc_url( BK_URI . '/assets/icons/' . $line_icon ); ?>" alt="" class="heading-line testimonials-heading-line" aria-hidden="true" />
      <p class="testimonials-desc" style="color: <?php echo esc_attr( $desc_color ); ?>;"><?php echo esc_html( $desc ); ?></p>
    </header>
    <?php else : ?>
