<?php
/**
 * سيكشن: blog (الصفحة الرئيسية)
 *
 * @package BrandKey
 */

// لو السيكشن مش مفعّل، اخرج
if ( ! bk_section_enabled( 'front_page', 'blog' ) ) return;

// محتوى السيكشن من Customizer
$title = bk_section( 'front_page', 'blog', 'title' );
$desc = bk_section( 'front_page', 'blog', 'desc' );
$heading = bk_section( 'front_page', 'blog', 'heading' ) ?: 'h2';
$bg = bk_section( 'front_page', 'blog', 'bg' );
$dark = ! empty( $bg );
$line_icon = $dark ? 'line-white.png' : 'heading-line.png';
$btn_text = bk_section( 'front_page', 'blog', 'btn_text' );
$btn_url = bk_section( 'front_page', 'blog', 'btn_url' );
...
<section class="blog" id="blog"<?php echo $style; ?>>
  <div class="blog-container">

    <?php if ( 'blog' !== 'hero' ) : ?>
    <header class="blog-head" id="blogHead">
      <<?php echo esc_html( $heading ); ?> class="blog-title" style="color: <?php echo esc_attr( $title_color ); ?>;"><?php echo esc_html( $title ); ?></<?php echo esc_html( $heading ); ?>>
      <img src="<?php echo esc_url( BK_URI . '/assets/icons/' . $line_icon ); ?>" alt="" class="heading-line blog-heading-line" aria-hidden="true" />
      <p class="blog-desc" style="color: <?php echo esc_attr( $desc_color ); ?>;"><?php echo esc_html( $desc ); ?></p>
    </header>
    <?php else : ?>
