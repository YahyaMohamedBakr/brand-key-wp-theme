<?php
/**
 * سيكشن: programs (training)
 * @package BrandKey
 */
if ( ! bk_section_enabled( 'training', 'programs' ) ) return;

$title = bk_section( 'training', 'programs', 'title' );
$desc = bk_section( 'training', 'programs', 'desc' );
$heading = bk_section( 'training', 'programs', 'heading' ) ?: 'h2';
$bg = bk_section( 'training', 'programs', 'bg' );
$dark = ! empty( $bg );
...
$btn_text = bk_section( 'training', 'programs', 'btn_text' );
$btn_url = bk_section( 'training', 'programs', 'btn_url' );
?>

<section class="training-programs" id="training-programs"<?php echo $style; ?>>
  <div class="container" style="max-width: 1280px; margin: 0 auto; padding: 90px 24px 80px;">

    <?php if ( 'programs' === 'hero' ) : ?>
