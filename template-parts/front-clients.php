<?php
/**
 * سيكشن: clients (الصفحة الرئيسية)
 *
 * @package BrandKey
 */

// لو السيكشن مش مفعّل، اخرج
if ( ! bk_section_enabled( 'front_page', 'clients' ) ) return;

// محتوى السيكشن من Customizer
$title = bk_section( 'front_page', 'clients', 'title' );
$desc = bk_section( 'front_page', 'clients', 'desc' );
$heading = bk_section( 'front_page', 'clients', 'heading' ) ?: 'h2';
$bg = bk_section( 'front_page', 'clients', 'bg' );
$dark = ! empty( $bg );
$line_icon = $dark ? 'line-white.png' : 'heading-line.png';
$btn_text = bk_section( 'front_page', 'clients', 'btn_text' );
$btn_url = bk_section( 'front_page', 'clients', 'btn_url' );
...
<section class="clients" id="clients"<?php echo $style; ?>>
  <div class="clients-container">

    <?php if ( 'clients' !== 'hero' ) : ?>
    <header class="clients-head" id="clientsHead">
      <<?php echo esc_html( $heading ); ?> class="clients-title" style="color: <?php echo esc_attr( $title_color ); ?>;"><?php echo esc_html( $title ); ?></<?php echo esc_html( $heading ); ?>>
      <img src="<?php echo esc_url( BK_URI . '/assets/icons/' . $line_icon ); ?>" alt="" class="heading-line clients-heading-line" aria-hidden="true" />
      <p class="clients-desc" style="color: <?php echo esc_attr( $desc_color ); ?>;"><?php echo esc_html( $desc ); ?></p>
    </header>
    <?php else : ?>
