<?php
/**
 * سيكشن: audience (training)
 * @package BrandKey
 */
if ( ! bk_section_enabled( 'training', 'audience' ) ) return;

$title = bk_section( 'training', 'audience', 'title' );
$desc = bk_section( 'training', 'audience', 'desc' );
$heading = bk_section( 'training', 'audience', 'heading' ) ?: 'h2';
$bg = bk_section( 'training', 'audience', 'bg' );
$dark = ! empty( $bg );
...
$btn_text = bk_section( 'training', 'audience', 'btn_text' );
$btn_url = bk_section( 'training', 'audience', 'btn_url' );
?>

<section class="training-audience" id="training-audience"<?php echo $style; ?>>
  <div class="container" style="max-width: 1280px; margin: 0 auto; padding: 90px 24px 80px;">

    <?php if ( 'audience' === 'hero' ) : ?>
