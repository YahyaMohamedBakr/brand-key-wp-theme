<?php
/**
 * سيكشن: problems (training)
 * @package BrandKey
 */
if ( ! bk_section_enabled( 'training', 'problems' ) ) return;

$title = bk_section( 'training', 'problems', 'title' );
$desc = bk_section( 'training', 'problems', 'desc' );
$heading = bk_section( 'training', 'problems', 'heading' ) ?: 'h2';
$bg = bk_section( 'training', 'problems', 'bg' );
$dark = ! empty( $bg );
...
$btn_text = bk_section( 'training', 'problems', 'btn_text' );
$btn_url = bk_section( 'training', 'problems', 'btn_url' );
?>

<section class="training-problems" id="training-problems"<?php echo $style; ?>>
  <div class="container" style="max-width: 1280px; margin: 0 auto; padding: 90px 24px 80px;">

    <?php if ( 'problems' === 'hero' ) : ?>
