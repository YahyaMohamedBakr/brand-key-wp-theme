<?php
/**
 * سيكشن: sectors (الصفحة الرئيسية)
 *
 * @package BrandKey
 */

// لو السيكشن مش مفعّل، اخرج
if ( ! bk_section_enabled( 'front_page', 'sectors' ) ) return;

// محتوى السيكشن من Customizer
$title = bk_section( 'front_page', 'sectors', 'title' );
$desc = bk_section( 'front_page', 'sectors', 'desc' );
$heading = bk_section( 'front_page', 'sectors', 'heading' ) ?: 'h2';
$bg = bk_section( 'front_page', 'sectors', 'bg' );
$dark = ! empty( $bg );
$line_icon = $dark ? 'line-white.png' : 'heading-line.png';
$btn_text = bk_section( 'front_page', 'sectors', 'btn_text' );
$btn_url = bk_section( 'front_page', 'sectors', 'btn_url' );
...
<section class="sectors" id="sectors"<?php echo $style; ?>>
  <div class="sectors-container">

    <?php if ( 'sectors' !== 'hero' ) : ?>
    <header class="sectors-head" id="sectorsHead">
      <<?php echo esc_html( $heading ); ?> class="sectors-title" style="color: <?php echo esc_attr( $title_color ); ?>;"><?php echo esc_html( $title ); ?></<?php echo esc_html( $heading ); ?>>
      <img src="<?php echo esc_url( BK_URI . '/assets/icons/' . $line_icon ); ?>" alt="" class="heading-line sectors-heading-line" aria-hidden="true" />
      <p class="sectors-desc" style="color: <?php echo esc_attr( $desc_color ); ?>;"><?php echo esc_html( $desc ); ?></p>
    </header>
    <?php else : ?>
