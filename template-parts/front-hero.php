<?php
/**
 * سيكشن: hero (الصفحة الرئيسية)
 *
 * @package BrandKey
 */

// لو السيكشن مش مفعّل، اخرج
if ( ! bk_section_enabled( 'front_page', 'hero' ) ) return;

// محتوى السيكشن من Customizer
$title = bk_section( 'front_page', 'hero', 'title' );
$desc = bk_section( 'front_page', 'hero', 'desc' );
$heading = bk_section( 'front_page', 'hero', 'heading' ) ?: 'h2';
$bg = bk_section( 'front_page', 'hero', 'bg' );
$dark = ! empty( $bg );
$line_icon = $dark ? 'line-white.png' : 'heading-line.png';
$btn_text = bk_section( 'front_page', 'hero', 'btn_text' );
$btn_url = bk_section( 'front_page', 'hero', 'btn_url' );
...
<section class="hero" id="hero"<?php echo $style; ?>>
  <div class="hero-container">

    <?php if ( 'hero' !== 'hero' ) : ?>
    <header class="hero-head" id="heroHead">
      <<?php echo esc_html( $heading ); ?> class="hero-title" style="color: <?php echo esc_attr( $title_color ); ?>;"><?php echo esc_html( $title ); ?></<?php echo esc_html( $heading ); ?>>
      <img src="<?php echo esc_url( BK_URI . '/assets/icons/' . $line_icon ); ?>" alt="" class="heading-line hero-heading-line" aria-hidden="true" />
      <p class="hero-desc" style="color: <?php echo esc_attr( $desc_color ); ?>;"><?php echo esc_html( $desc ); ?></p>
    </header>
    <?php else : ?>
    <div class="hero-container">
