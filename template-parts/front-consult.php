<?php
/**
 * سيكشن: consult (الصفحة الرئيسية)
 *
 * @package BrandKey
 */

// لو السيكشن مش مفعّل، اخرج
if ( ! bk_section_enabled( 'front_page', 'consult' ) ) return;

// محتوى السيكشن من Customizer
$title = bk_section( 'front_page', 'consult', 'title' );
$desc = bk_section( 'front_page', 'consult', 'desc' );
$heading = bk_section( 'front_page', 'consult', 'heading' ) ?: 'h2';
$bg = bk_section( 'front_page', 'consult', 'bg' );
$dark = ! empty( $bg );
$line_icon = $dark ? 'line-white.png' : 'heading-line.png';
$btn_text = bk_section( 'front_page', 'consult', 'btn_text' );
$btn_url = bk_section( 'front_page', 'consult', 'btn_url' );
...
<section class="consult" id="consult"<?php echo $style; ?>>
  <div class="consult-container">

    <?php if ( 'consult' !== 'hero' ) : ?>
    <header class="consult-head" id="consultHead">
      <<?php echo esc_html( $heading ); ?> class="consult-title" style="color: <?php echo esc_attr( $title_color ); ?>;"><?php echo esc_html( $title ); ?></<?php echo esc_html( $heading ); ?>>
      <img src="<?php echo esc_url( BK_URI . '/assets/icons/' . $line_icon ); ?>" alt="" class="heading-line consult-heading-line" aria-hidden="true" />
      <p class="consult-desc" style="color: <?php echo esc_attr( $desc_color ); ?>;"><?php echo esc_html( $desc ); ?></p>
    </header>
    <?php else : ?>
